...
- **FR-006**: By default, the installed alias MUST be the manifest's `name`. When that alias already maps to a different origin in the local registry, installation MUST fail explicitly, naming the conflicting alias, and MUST NOT overwrite, rename, or silently auto-suffix it; reinstalling the same origin under the same existing alias MUST succeed idempotently. An explicit alternative alias MUST let the user resolve the conflict at install time.
