...
5. **Given** an installed plugin whose manifest declared components or conventions that a later version of the same origin no longer declares, **When** the user reinstalls that origin under the same alias, **Then** the registry holds exactly what the new manifest declares for that alias, and nothing the previous version declared but the new one dropped remains registered.
...
5. **Given** the Starter-resolved branch exists in the repository only as a remote-tracking branch (for example a fresh clone that never checked it out), **When** the user selects contribution mode, **Then** Work checks out a local branch of that name tracking the remote one — never a detached HEAD — and the snapshot's branch is that local branch name, without a remote prefix.
...
- A manifest `name`, or an `--as` value, is not a valid alias (empty, contains a path separator, is `.` or `..`, or would collide with a package's replacement backup): installation fails as an invalid plugin before anything is registered or written under plugin storage.
- A Starter component declares a `pattern` that is not a valid regular expression: installation fails as an invalid plugin, registering nothing, rather than installing a Starter that can never match.
- `--as` (or a manifest `name`) names the reference package's alias: an alias conflict, exactly as for any other installed plugin, though the reference package is not installed through `work plugin install`.
- A plugin is reinstalled from the same origin after its manifest dropped a component or convention: only the new manifest's entries remain for that alias; a convention name still declared by another installed plugin stays in the catalog.
- Contribution mode's branch exists locally already: that local branch is checked out and, on failure or cancellation, is never deleted. It exists only as a remote-tracking branch: a local tracking branch is created for the checkout and, being created by this run, is removed again on failure or cancellation.
- A Work created in contribution mode has no slug: every place that identifies a Work to the user (pickers, receipts, confirmations) identifies it by its branch instead, and none renders a blank name.
...
- **FR-004a**: A Starter component's `pattern`, when present, MUST be a valid regular expression in the syntax Work evaluates it with (Go); installation MUST reject the whole installation as an invalid plugin when it is not, so a registered `pattern` is always one Work can evaluate.
- **FR-004b**: The installed alias, whether taken from the manifest's `name` or from `--as`, MUST be a single, non-empty path segment beginning with an ASCII letter or digit and containing only ASCII letters, digits, `.`, `_` and `-`, and MUST NOT end in `.old`. Installation MUST reject any other value as an invalid plugin before touching plugin storage. The alias is used as a directory name under plugin storage; this grammar keeps it from escaping that directory or colliding with the replacement backup of another alias.
...
- **FR-020**: In contribution mode, Work MUST check out the Starter-resolved branch directly, MUST NOT run the slug, convention, or prefix steps, MUST NOT require that branch to be new, and MUST persist the start mode and branch without a `branch_convention` value. The checkout MUST be of a local branch on which work can be committed: when the resolved branch exists only as a remote-tracking branch, Work MUST create a local branch of that name tracking it and check that out, never leaving a detached HEAD, and the persisted branch MUST be that local branch name, never a `<remote>/<name>` reference.
- **FR-020a**: A Work with no slug (contribution mode) MUST be identified to the user by its branch wherever Work identifies a Work by name — pickers, receipts, and confirmations — and MUST NOT be shown with a blank name.
...
- **FR-033**: A `work start` invocation that fails or is cancelled at any point (Starter match, Starter execution, resolution, mode/branch/convention/slug selection, branch-name validation) MUST leave no branch, worktree, Work directory, snapshot, or index entry behind. A branch that existed before the invocation (including the local branch contribution mode checks out) is never deleted by it; a branch the invocation itself created (a new Work's branch, or the local tracking branch of FR-020) is removed.
...
- **SC-004**: An alias conflict at install time — including against the reference package's alias — fails in 100% of cases with zero silent overwrite, rename, or auto-suffix; reinstalling the same origin under the same alias succeeds idempotently in 100% of cases, and leaves the registry equal to the new manifest's declarations.
- **SC-005**: A manifest violating role-based field validation, declaring an invalid Starter `pattern`, or yielding an invalid alias is rejected before registering any of its components or writing to plugin storage in 100% of test fixtures — zero partial registrations.
