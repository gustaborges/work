...
- [X] T035 [P] [US1] Implement `internal/workspace/workspace.go`: `SuggestDefault()` = `~/work` (`%USERPROFILE%\work` on Windows); `Validate(path)` per research R8 (writable dir or creatable; not inside a git work tree; not inside any `repository_roots`); `Persist(path)` writes `config.workspace` + creates `<root>/in-progress/` + `<root>/archived/`. Tests.
