...
- [X] T069 Guided-interview polish in `internal/cli/start.go` + `internal/tui`: reorder the interactive steps to slug → base branch → workspace root (slug/name validation, the cheapest correction, now precedes the base-branch scan; workspace resolution stays deferred to just before confirm per SC-004); and replace each resolved step's live prompt with a compact completed widget — the step title then `✓ <value>`, one blank line before the next prompt — via `tui.StepDone`. The base-branch picker (`basebranch_picker.go`) collapses to an empty frame on select/abort instead of leaving the list on screen. Steps supplied by flag and the single-prefix `freeform` case render no widget. `contracts/cli-work-start.md` interactive-flow steps renumbered to match; `plan.md`/`research.md` pipeline notes and the `README.md`/`quickstart.md` walkthroughs reordered. Tests: `tui` `StepDone`/collapsed-view units, expanded pty assertions in `tests/integration/interactive_test.go`. (FR-008, FR-009, FR-013, SC-004, SC-008.)
