...
1. **Given** a clean installation, a compatible interactive terminal, and a path to a valid local Git clone, **When** the user runs `work start <path>` and supplies the guided choices it requests — a slug, a base branch, and (accepting or changing the suggested value) a workspace root, with the `freeform` convention contributing its single prefix — **Then** a Work in `in-progress` state is created with its own branch, isolated worktree, canonical snapshot, and coherent lookup record, and the session ends positioned in the new checkout.
...
- **FR-006**: When no workspace root is configured, the product MUST suggest a value, allow changing it, validate its fitness to hold Works, and persist the choice before the first creation. The product MUST NOT reject a workspace root solely because a Git repository encloses it; separation is required only from configured source-clone locations.
...
- **FR-009**: The product MUST let the user select an exact base revision from the repository's local and remote branches, presented so that homonymous or divergent references remain distinguishable. In the guided flow the selection MAY be staged — first the branch source (remote or local), then the branch.
