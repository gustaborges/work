...
**Requisitos atendidos (referência, não reinterpretação):** `docs-v2/prd.md` — RF-1 a RF-22, RNF-1 a RNF-7. Este documento descreve como a arquitetura é estruturada para satisfazer esses requisitos; não redefine nem reinterpreta o que eles pedem. Uma incompatibilidade encontrada aqui é motivo para revisar o PRD ou uma ADR, nunca para o design divergir silenciosamente.
...
O estado do sistema vive inteiramente em disco, sob `~/.work/` (plugins, config, registro) e `~/workspace/` (worktrees e contexto de cada Work) — ver Seções 3 e 5.
