...
* **Agrupar a troca de convenção sob `work plugin`.** Rejeitada: mistura gestão de ciclo de vida de plugin (instalar/habilitar/atualizar) com uma escolha do usuário sobre um repositório — dois modelos mentais diferentes. Um comando de topo dedicado (`work convention`) é mais direto do que qualquer agrupamento, já que é uma escolha de configuração do repositório com comando próprio.
