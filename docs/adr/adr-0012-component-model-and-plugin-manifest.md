...
**Trade-offs:** o manifesto fica mais expressivo e requer validação discriminada por role; extensões automáticas podem falhar depois da criação do Work, portanto o Work conclui com aviso em vez de desfazer um ambiente já materializado.

## Relação com decisões anteriores

Esta ADR consolida as decisões de pacote/manifesto e convenções declarativas antes registradas separadamente. Ela substitui o modelo que usava `type`, `capabilities` ou `hooks`, bem como o contrato de hook anterior. As decisões de subprocessos externos, runtime explícito e colisão de Starters continuam em ADRs próprios.
